...
int startingspeed = 500; // starting speed(ms).  
int charPos = 1;
int scrollhelp = 2;
int score = 0;
int scoredigits = 1;
int charHeight = 1;
int speed = 500; // variable for how fast the terrain moves, constantly decreasing, making game faster
...
//#define emptyterrainbyte 1 code from website
//#define rightterrainbyte 2
//#define leftterrainbyte 3
//#define terrainbyte 4
...
//  blockfourlength = 0;
  ...
  lcd.write(byte(5));
  lcd.setCursor((20+blockonelength), blocktwoheight);
  lcd.write(byte(3));
  ...
  lcd.setCursor((26+blockonelength+blocktwolength), blockthreeheight);
  lcd.write(byte(3));
  for (int i = 0; i < blockthreelength; i++) {
    lcd.write(byte(4));
  }
  lcd.write(byte(5));
  if ((32 + blockonelength+blocktwolength+blockthreelength+blockfourlength) < 38) {
    lcd.setCursor(32+blockonelength+blocktwolength+blockthreelength+blockfourlength, blockfourheight);
    lcd.write(byte(3));
...
void detectCollision() {
  ...
  (20+blockonelength) <= charPos && charPos <= (22+blockonelength+blocktwolength) && (onLower == false && blocktwoheight == 1) ||
  (26+blockonelength+blocktwolength) <= charPos && charPos <= (28+blockonelength+blocktwolength+blockthreelength) && (onLower == false && blockthreeheight == 1) ||
  (32+blockonelength+blocktwolength+blockthreelength) <= charPos && charPos <= (34+blockonelength+blocktwolength+blockthreelength+blockfourlength) && (onLower == false && blockfourheight == 1)) {
    onTerrain = true;
    } else if(jumpingposition == 4) {
      falling = true; fallingposition = 1;
    }
    else if ((onLower == false) && ((charPos == 17+blockonelength) || (charPos == 23+blockonelength+blocktwolength) ||
    (charPos == 29+blockonelength+blocktwolength+blockthreelength) || (charPos == 35+blockonelength+blocktwolength+blockthreelength+blockfourlength || charPos == 0))) {
      falling = true;
      fallingposition = 2;
      onTerrain = false;
      onGround = false;
  ...
  (20+blockonelength) <= charPos && charPos <= (22+blockonelength+blocktwolength) && ((onLower == true && blocktwoheight == 1) || (onUpper == true && blocktwoheight == 0)) || 
  (26+blockonelength+blocktwolength) <= charPos && charPos <= (28+blockonelength+blocktwolength+blockthreelength) && ((onLower == true && blockthreeheight == 1) || (onUpper == true && blockthreeheight == 0)) ||
  (32+blockonelength+blocktwolength+blockthreelength) <= charPos && charPos <= (34+blockonelength+blocktwolength+blockthreelength+blockfourlength) 
...
//  lcd.write(byte(5));
  Serial.begin(1200);
//  generateTerrain();
//  updateLowerCharacter();
//  updateUpperCharacter();
  lcd.setCursor(2, 0);
...
//  }
  ...
  charPos++;
//  byte newTerrainType = emptyterrainbyte; terrain from website
...
//    }
//  }
//  advanceTerrain(17, newTerrainType == 1 ? terrainbyte : emptyterrainbyte);    Copied code from website
//  advanceTerrain(17, newTerrainType == 1 ? terrainbyte : emptyterrainbyte);
